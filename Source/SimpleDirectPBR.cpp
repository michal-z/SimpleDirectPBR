...
static void Initialize(FDemoRoot& Root, eastl::vector<ID3D12Resource*>& OutStagingResources)
{
	FGraphicsContext& Gfx = Root.Gfx;
...
static int32_t Run(FDemoRoot& Root)
{
	...
	HWND Window = CreateSimpleWindow("SimpleDirectPBR", 1920, 1080);
	CreateGraphicsContext(Window, false, Root.Gfx);

	eastl::vector<ID3D12Resource*> StagingResources;

	CreateUIContext(Root.Gfx, Root.UI, 8, StagingResources);
	Initialize(Root, StagingResources);
	...
	for (;;)
	{
		...
		else
		{
			double Time;
			float DeltaTime;
			UpdateFrameStats(Window, "SimpleDirectPBR", Time, DeltaTime);
			UpdateUI(DeltaTime);
